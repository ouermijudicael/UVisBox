...
.. code-block:: python
    ...
    def double_gyre(x, y, t=0):
        ...
        f = a * x**2 + b * x
        df_dx = 2 * a * x + b
        u = -np.pi * A * np.sin(np.pi * f) * np.cos
        v = np.pi * A * np.cos(np.pi * f) * np.sin(np.pi * y) * df_dx
        return u, v

Create a grid of points and compute the double gyre flow vectors at those points.
...
Plot the original vector field and the uncertainty squid glyphs
...
# Plot uncertainty squid glyphs
ax2 = uncertainty_squid_glyphs_2D(positions, ensemble_vectors, 1.0, scale=0.4, ax=ax2)
ax2.set_title("Uncertainty Squid Glyphs for Double Gyre Flow")
ax2.set_xlim(-0.25, 2.25)
ax2.set_ylim(-0.25, 1.25)
ax2.set_xlabel("X")
...
plt.tight_layout()
plt.savefig("squid_glyphs_2D_example.png", dpi=300)
plt.show()